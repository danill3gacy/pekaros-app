...
def _hint(err):
    ...
    if any(w in s for w in ("connection refused", "refused", "max retries", "failed to establish",
                            "connection error", "newconnectionerror", "[errno 61]", "[errno 111]",
                            "actively refused", "cannot connect", "connecterror")):
        if local:
            return ("🔴 ИИ не отвечает: похоже, Ollama не запущена.\n"
    ...
    if any(w in s for w in ("rate limit", "429", "too many requests", "quota", "insufficient_quota")):
        return ("🟠 Сервер модели ограничил частоту запросов. Подождите минуту и "
    ...
    if any(w in s for w in ("not found", "404", "no such model", "model not found",
                            "does not exist", "unknown model")):
        return (f"🔴 Модель «{model}» не установлена.\n"
                f"Скачайте её один раз: ollama pull {model}")
    if any(w in s for w in ("timed out", "timeout", "read timed out")):
        return ("🟠 ИИ думает слишком долго. Для qwen2.5:3b на слабом железе это бывает — "
...
def ping(timeout=4):
    ...
    try:
        ...
        if want and models and want not in models:
            ...
            return {"ok": False, "configured": True, "models": models,
                    "reason": f"Модель «{want}» не найдена.{hint} Скачайте: ollama pull {want}"}
        return {"ok": True, "configured": True, "models": models,
                "reason": f"{want} готова" + (f" (моделей на сервере: {len(models)})" if models else "")}
...
def complete(system, user, timeout=None, max_tokens=500, temperature=0.3):
    ...
    try:
        client = OpenAI(api_key=config.LLM_API_KEY or "local",
