...
def main():
    ...
    if cmd == "seed":
        ...
        fn = {
            "nedelya": orchestrator.weekly_brief,
            ...
            "upusheno": orchestrator.answer_lost,
        }[cmd]
        conn = db.get_conn()
        ...
        conn.close()
        print(
            f"Пересобран разбор: уникальных названий {res['names']}, обновлено строк {res['rows']}"
        )
    elif cmd == "ask":
        from . import db, orchestrator
        ...
        if res.get("bad_date"):
            print(
                f"  ⚠ строк с непонятной датой: {res['bad_date']} — проверьте формат колонки даты"
            )
        if res.get("returns"):
            print(f"  ↩ возвратов учтено (вычтены из выручки): {res['returns']}")
        if res.get("no_payment_column"):
            print(
                "  ⚠ в выгрузке нет колонки типа оплаты — всё зачтено как безнал, "
        ...
        if res.get("no_time_column"):
            print(
                "  ⚠ в выгрузке нет ВРЕМЕНИ продажи, только дата.\n"
        ...
        if res.get("no_receipt_column"):
            print(
                "  ⚠ в выгрузке нет номера чека — состав чеков не восстановить.\n"
                "     Число чеков и средний чек будут неточными."
            )
        if res.get("service_items"):
            print(
                f"  ℹ служебных позиций (пакеты, скидки) исключено из расчётов: {res['service_items']}"
            )
    elif cmd == "backup":
        from . import backup
