...
def answer_case(conn):
    ...
    for it in p["items"][:12]:
        ...
        lines.append(f"{arrow} {_p(it['name'])}: *{it['recommended']}* шт  "
                     f"(продаётся ~{it['sold_avg']}, {it['note']})")
    lines.append("")
    lines.append("_Заказ намеренно выше среднего спроса: пустая витрина стоит дороже "
...
def answer_margin(conn):
    ...
    lines.append(f"Из неё посчитано {_pct(t['coverage'])}: сырьё {fmt(t['cost'])} ₽ · "
                 f"маржа *{fmt(t['margin'])} ₽*")
    lines.append(f"Фудкост *{_pct(t['foodcost'], 1)}* "
                 f"(ориентир {_pct(config.TARGET_FOODCOST)})")
    if t["coverage"] < 0.97:
        lines.append("_Остальное — товары на полке, добавки отдельной строкой и "
    ...
    if bad:
        ...
        for i in bad:
            lines.append(f"• {_p(i['name'])} — фудкост {_pct(i['foodcost'])}, "
    ...
    if m["items"]:
        ...
        titles = {costing.STAR: "⭐ Держат заведение",
...
def answer_milk(conn):
    ...
    if not rows:
        return ("🥛 В данных нет разбивки по молоку: касса не передаёт модификатор. "
                "Тогда себестоимость считается по обычному молоку, и маржа на "
                "растительном не видна.")
    lines = ["🥛 *Молоко: сколько зарабатываете*", ""]
    for r in rows:
        lines.append(f"• {r['milk']} — {_pct(r['share'])} чашек · цена {fmt(r['price'])} ₽ · "
    ...
    if reg and alts:
        ...
        lines.append("")
        if diff < 0:
            lines.append(f"↳ На {worst['milk']} молоке вы зарабатываете на "
                         f"*{fmt(-diff)} ₽ меньше* с чашки, чем на обычном. "
                         f"Наценку стоит пересмотреть.")
        else:
            lines.append(f"↳ Наценка за альтернативное молоко покрывает разницу: "
...
def answer_supply(conn):
    ...
    if urgent:
        lines.append("*Горит:*")
        for i in urgent[:5]:
            lines.append(f"🔴 {_p(i['name'])} — хватит на {i['days_left']} дн., "
    ...
    for i in rep["items"][:16]:
        ...
        lines.append(f"• {_p(i['name'])} — *{i['packs']}* {i['pack_name']} "
    ...
    if short_cycle:
        ...
        for i in short_cycle:
            lines.append(f"• {_p(i['name'])} — каждые {i['review_days']:.0f} дн. "
...
def answer_attach(conn):
    ...
    if b:
        ...
        for x in b[:4]:
            lines.append(f"• {_p(x['drink'])} + {_p(x['food'])} — в {x['checks']} чеках "
...
def answer_drinks(conn):
    ...
    if ic["total"]:
        ...
        if len(ic["by_month"]) >= 2:
            ...
            lines.append(f"  ({first['month']}: {_pct(first['share'])} → "
...
def answer_shift(conn):
    ...
    lines = [f"👥 *Смена на {f['target']}*", "",
             f"Ожидается *{f['low']}–{f['high']} чеков*."]
    if sp.get("peaks"):
        lines.append("🕗 Пик: " + " и ".join(f"{p['hour']:02d}:00" for p in sp["peaks"]) +
                     f" · тише всего в {sp['quietest']['hour']:02d}:00")
    cap = sp.get("capacity") or {}
    if cap.get("known"):
        lines.append(f"Ваш потолок — около *{cap['cups_per_hour']:.0f} чашек в час* "
    ...
    if sp.get("scenario"):
        sc = sp["scenario"]
        lines += ["", f"В эти часы вы упираетесь в потолок в {_pct(sc['clipped_share'])} "
...
def answer_lost(conn):
    ...
    if day["items"]:
        ...
        for i in day["items"][:6]:
            ...
            lines.append(f"• {_p(i['name'])} {h} — не продали ~{i['lost_units']} шт "
    ...
    if not rep["items"]:
        ...
        return "\n".join(lines)
    lines.append(f"*В среднем: {fmt(rep['total_per_day'])} ₽ в день "
                 f"≈ {fmt(rep['total_per_month'])} ₽ в месяц.*")
    lines.append("")
    for i in rep["items"][:6]:
        lines.append(f"• {_p(i['name'])} — кончался {i['sellout_days']} дн., "
                     f"~{fmt(i['lost_money_per_month'])} ₽/мес")
    lines.append("")
    lines.append("Этих денег нет в кассе: гость не нашёл завтрак, взял только кофе — "
...
def answer_waste(conn):
    ...
    if w["case"]:
        lines.append(f"*Витрина* — {fmt(w['case_total'])} ₽ по ценникам "
                     f"({w['case_pct']:.1f}% выручки):")
        for i in w["case"][:8]:
    ...
    if not w["items"]:
        ...
        lines.append("")
        lines.append("Это не всегда хорошая новость: если витрина кончается до закрытия, "
                     "вы теряете больше, чем на списаниях. Спросите «сколько недопродали».")
    if mw.get("tracked"):
        lines += ["", f"🥛 Вылитого молока в среднем {mw['litres_per_day']} л в день — "
                      f"около *{fmt(mw['money_per_month'])} ₽ в месяц*."]
    else:
        lines += ["", "_" + mw["note"] + "_"]
    lines += ["", "Отметить: «списание круассаны 4» или «вылил молоко 1,5». "
...
def answer_audit(conn):
    ...
    lines.append(f"💵 Выручка *{fmt(s['revenue'])} ₽* · {s['checks']} чеков · "
                 f"чек {fmt(s['avg'])} ₽ · {s['cups']:.0f} чашек")
    if t["revenue_costed"]:
        lines.append(f"💰 Фудкост за период *{_pct(t['foodcost'], 1)}* · "
                     f"маржа {fmt(t['margin'])} ₽")
    lines.append("")
    lines.append("*Где ваши деньги:*")
    if lost["items"]:
        top = lost["items"][0]
        lines.append(f"💸 Недопродали на *{fmt(lost['total'])} ₽* — кончилось "
    ...
    sp = staffing.shift_plan(conn)
    if sp.get("scenario"):
        lines.append(f"👥 В {', '.join(f'{h}:00' for h in sp['scenario']['confirmed_hours'])} "
...
def answer_cash(conn):
    last = last_day_with_data(conn)
    s = analytics.sales_summary(conn, last)
    return (f"🧾 *Сверка кассы за {last.isoformat()}*\n\n"
            f"💳 Безнал: {fmt(s['card'])} ₽\n"
...
HANDLERS = {
    "case": answer_case, "margin": answer_margin, "supply": answer_supply,
    "attach": answer_attach, "drinks": answer_drinks, "shift": answer_shift,
    "lost": answer_lost, "waste": answer_waste, "audit": answer_audit,
    "revenue": answer_revenue, "cash": answer_cash,
}

# точные подписи кнопок и коротких команд -> мгновенный расчёт (без ИИ)
BUTTONS = {
    "🥐 витрина на завтра": "case", "витрина": "case", "витрина на завтра": "case",
    "💰 маржа и меню": "margin", "маржа": "margin", "маржа и меню": "margin",
    "📦 заявка поставщику": "supply", "заявка поставщику": "supply", "закупки": "supply",
    "🥐 еда к кофе": "attach", "еда к кофе": "attach", "attach": "attach",
    "☕ что пьют": "drinks", "что пьют": "drinks",
    "💸 упущенная выручка": "lost", "упущенная выручка": "lost",
    "сколько недопродали": "lost", "недопродажи": "lost",
    "👥 смена": "shift", "смена": "shift", "смена завтра": "shift",
    "🗑 списания": "waste", "списания": "waste",
    "📊 экспресс-аудит": "audit", "экспресс-аудит": "audit", "аудит": "audit",
    "📈 выручка": "revenue", "выручка": "revenue",
    "🧾 сверка кассы": "cash", "сверка кассы": "cash",
    "🥛 молоко": "milk", "молоко": "milk",
}
...
def answer(conn, text, allow_write=True):
    ...
    if intent:
        return HANDLERS[intent](conn)
    return ("Могу ответить по вашей кофейне: продажи и средний чек, маржа и фудкост, "
...
def _handle_writes(conn, low, allow_write):
    ...
    if m:
        ...
        if res and res.get("ambiguous"):
            return _ask_which(m.group(1).strip(), res["ambiguous"],
                              f"списание <нужное> {m.group(2)}")
        if res is None:
        ...
        return (f"✅ Записал: {_p(res['name'])} — {qty} {res['unit']} "
                f"({fmt(res['amount'])} ₽).{note}")
    ...
    if (re.match(r"^\s*(?:списани[ея]|вылил)\s+\S", low) and re.search(r"\d", low)
            and not re.search(r"\bза\b|\bпо\b|покажи|какие|сколько|почему|отч[её]т", low)):
        return ("Не понял формат. Напишите: «списание круассаны 4» или "
    ...
    if m:
        ...
        if res and res.get("error") == "implausible":
            return (f"Столько под стойкой не помещается. Проверьте число: "
    ...
    if m:
        ...
        if price <= 0:
            return ("Цена должна быть больше нуля: «цена зерно 1800» — "
        ...
        name = mi["name"]
        row = conn.execute("SELECT pack_name FROM ingredients WHERE name=?",
                           (name,)).fetchone()
    ...
    if c and not c.group(2).startswith("на завтра"):
        ...
        if res and res.get("ambiguous"):
            return _ask_which(c.group(2), res["ambiguous"],
        ...
        if res.get("error") == "not_food":
            return (f"{_p(res['name'])} — это не витрина, а «{res['kind']}». "
                    f"Напиток кончиться не может, его в заказ витрины ставить нельзя.")
        return (f"✅ {_p(res['name'])} — {'в витрине' if want else 'НЕ в витрине'}.\n"
...
def _catalog_text(conn):
    ...
    titles = {menu.KIND_DRINK: "☕ Напитки", menu.KIND_FOOD: "🥐 Витрина",
              menu.KIND_GOODS: "🛍 Товары", menu.KIND_ADDON: "➕ Добавки",
              menu.KIND_SERVICE: "⚙️ Служебные"}
    lines = ["📋 *Каталог*"]
    for kind in (menu.KIND_DRINK, menu.KIND_FOOD, menu.KIND_GOODS,
...
def _prices_text(conn):
    ...
    if default:
        ...
        lines += [f"• {_p(r['name'])} — {r['pack_price']:g} ₽ за {r['pack_name']}"
                  for r in default[:15]]
        lines.append("")
    if owner:
        lines.append("*Ваши:*")
...
def _maintenance_text(conn):
    ...
    for d in due[:10]:
        when = "ни разу не отмечали" if d["days_since"] is None \
...
def _waste_input_help(conn, what, qty_text):
    ...
    if qty is not None and qty > catalog.MAX_WASTE_QTY:
        return (f"Многовато для одного дня — {qty:g}. Если это не опечатка, "
                f"внесите несколькими сообщениями.")
    if len(what) < catalog.MIN_STEM:
        return ("Слишком короткое название — по одной-двум буквам не опознать. "
                "Напишите как в чеке: «списание круассан 4».")
    return (f"Не нашёл «{what}» ни в витрине, ни в сырье. "
            f"Список позиций — «каталог», список сырья — «цены».")


def help_text():
    return ("Я — КофейняОС. Вот что я умею:\n\n"
...
def morning_brief(conn):
    ...
    lines.append(f"💵 Выручка *{fmt(s['revenue'])} ₽* "
    ...
    if lost["items"]:
        ...
        lines.append(f"💸 Недопродали ~*{fmt(lost['total'])} ₽*: кончился "
                     f"{_p(top['name'])}{when}")
    if w["total"]:
        lines.append(f"🗑 Списано на {fmt(w['total'])} ₽")
    ...
    if ups:
        lines.append("🥐 Сегодня в витрину больше: " +
                     ", ".join(f"{_p(i['name'])} — {i['recommended']} шт" for i in ups))
    urgent = [i for i in supply.reorder(conn)["items"]
              if i["urgency"] in ("critical", "soon")][:3]
    if urgent:
        lines.append("📦 Заканчивается: " +
...
def weekly_brief(conn):
    ...
    prev_week = sum(analytics.sales_summary(conn, last - timedelta(days=7 + i))["revenue"]
                    for i in range(7))
    delta = ((total - prev_week) / prev_week * 100) if prev_week else 0
    lines = [f"📅 *Итоги недели* (по {last.isoformat()})", ""]
    lines.append(f"💵 Выручка за 7 дней: *{fmt(total)} ₽* "
                 f"({'+' if delta >= 0 else ''}{delta:.0f}% к пред. неделе)")
    best = max(week, key=lambda d: d["revenue"])
    lines.append(f"🔝 Лучший день: {analytics.WEEKDAY_SHORT[best['dow']]} — "
                 f"{fmt(best['revenue'])} ₽")
    t = costing.totals(conn, days=7, upto=last)
    if t["revenue_costed"]:
        lines.append(f"💰 Фудкост недели {_pct(t['foodcost'], 1)} · "
                     f"маржа {fmt(t['margin'])} ₽")
...
_QUESTION_WORDS = ("почему", "зачем", "стоит ли", "посовет", "как лучше", "что делать",
                   "можно ли", "а если", "сравни", "объясни", "насколько", "прогноз на",
                   "что думаешь", "как думаешь", "лучше ли", "выгодн")
...
def local_analytics(conn, text):
    ...
    if not any(w in low for w in (
            "чек", "день", "дням", "недел", "час", "пик", "во сколько", "прода",
            "берут", "покупа", "популярн", "ходов", "топ", "хит", "бестселлер",
            "дела", "как бизнес", "как идут", "как обстоят", "картина", "чаще",
            "идёт", "идет", "хват", "зерн", "чашек", "чашк", "фудкост", "марж",
            "зарабат", "себестоим", "наценк", "выгодн", "стоит", "почём", "почем")):
        return None
    ...
    if has("марж", "фудкост", "себестоим", "наценк",
           "сколько зараба", "выгодн", "сколько стоит", "почём", "почем"):
    ...
    if has("хват") and has("зерн", "молок", "сироп", "стакан", "кофе"):
        ...
        if not known:
            return ("Остатки не пересчитывали, поэтому «на сколько хватит» я сказать не могу — "
    ...
    if has("день недели", "дни недели", "по дням недел", "дню недели", "какой день недел"):
        ...
        if has("меньше", "слаб", "хуже", "мало", "тих", "наимень"):
            return (f"Самый слабый день недели — {s['weekday']}: в среднем {s['avg_checks']} "
                    f"чеков и {fmt(s['avg_revenue'])} ₽ в день.")
        return (f"Больше всего у вас {b['weekday']}: в среднем {b['avg_checks']} чеков и "
                f"{fmt(b['avg_revenue'])} ₽ в день. Самый слабый — {s['weekday']} "
                f"(~{s['avg_checks']} чеков).")

    if (re.search(r"\bпик\b", low)
            or has("по часам", "в каком часу", "в котором часу", "час пик")
            or ("во сколько" in low and any(w in low for w in [
                "прода", "выручк", "народ", "поток", "люд", "покупа", "больше всего",
                "загруж", "трафик", "заказ", "очеред"]))):
    ...
    if (("лучше всего" in low and has("прода", "берут", "идёт", "идет", "поку"))
            or has("популярн", "ходов", "топ товар", "что берут", "хиты", "хит прода",
                   "бестселлер", "больше всего прода", "что покупают", "что чаще всего")):
        tp = d["top_products"][:3]
        return ("Лучше всего продаются: "
                + ", ".join(f"{_p(x['name'])} (~{x['qty']} шт за период)" for x in tp) + ".")

    if "недел" in low and (re.search(r"как\s+(?:\w+\s+){0,2}недел", low)
                           or has("итог", "обзор недел", "результат", "сводка за недел",
                                  "прошлую недел", "прошлой недел", "недельн")):
        return weekly_brief(conn)

    if has("как дела", "как бизнес", "как идут", "как обстоят", "общая картина",
           "как у нас дела", "как продажи"):
        base = (f"За {d['period']['days']} дней: выручка {fmt(t['revenue'])} ₽, "
...
def _item_in_text(conn, low):
    ...
    tokens = [w for w in re.findall(r"[а-яёa-z]+", low)
              if len(w) >= 3 and w not in _STOPWORDS]
    if not tokens:
        return None
    names = [r["name"] for r in conn.execute(
        "SELECT name FROM menu_items WHERE kind IN ('drink','food')")]
    ...
    def score(n):
        return sum(1 for w in n.lower().split()
                   if any(matches(w, t) for t in tokens))
...
_STOPWORDS = {
    "какая", "какой", "какое", "какие", "каков", "сколько", "почему", "зачем",
    "когда", "где", "что", "чем", "как", "это", "мне", "нам", "для", "при",
    "над", "под", "без", "про", "его", "её", "их", "мой", "наш", "меня",
    "маржа", "маржи", "маржу", "цена", "цены", "фудкост", "наценка", "наценку",
    "себестоимость", "выгодно", "зарабатываем", "зарабатываю", "зарабатывает",
    "продано", "выручка", "сейчас", "тогда", "лучше", "хуже",
}
...
def _item_money_answer(conn, name):
    ...
    if not row["cost_known"]:
        ...
        return (f"{_p(name)}: продано {row['qty']:.0f}, выручка {fmt(row['revenue'])} ₽, "
    ...
    return (f"{_p(name)}: цена {row['price']:.0f} ₽, сырьё {row['cost']:.0f} ₽, "
...
def _digest_uncached(conn, days=56):
    ...
    tot = conn.execute(
        ...
        (analytics.day_str(start), analytics.day_str(last))).fetchone()
    ndays = tot["days"] or 1
    revenue = tot["rev"] or 0
    checks = tot["checks"] or 0
    wd = analytics.weekday_breakdown(conn, days, last)
    wd_by_checks = sorted([x for x in wd if x["avg_checks"]],
                          key=lambda x: x["avg_checks"], reverse=True)
    ...
    w = conn.execute(
        ...
        (analytics.day_str(start), analytics.day_str(last))).fetchone()
    return {
        "period": {"from": analytics.day_str(start), "to": analytics.day_str(last),
                   "days": ndays},
        "totals": {"revenue": round(revenue), "checks": checks,
...
def _digest_text(conn):
    ...
    L = [f"Заведение: кофейня «{config.VENUE_NAME}».",
    ...
    L.append("Напитки за период (чашек / доля): " +
             "; ".join(f"{x['name']} — {x['qty']} шт / {round(x['share'] * 100)}%"
                       for x in d["drink_mix"]))
    L.append("Топ позиций (продано / выручка ₽ / цена за штуку ₽): " +
             "; ".join(f"{x['name']} — {x['qty']}/{x['rev']}/"
                       f"{round(x['rev'] / x['qty']) if x['qty'] else 0}"
                       for x in d["top_products"]))
    L.append("Доли категорий в выручке: " +
             "; ".join(f"{c['cat']} {c['share']}%" for c in d["by_category"]))

    if d["foodcost"] is not None:
        L.append(f"Фудкост (доля сырья в выручке): {round(d['foodcost'] * 100, 1)}%, "
                 f"маржа за период {d['margin']} ₽.")
        econ = costing.item_economics(conn)
        known = [i for i in econ if i["cost_known"]][:10]
        if known:
            L.append("Маржа по позициям (цена ₽ / себестоимость ₽ / маржа ₽ / фудкост %): " +
                     "; ".join(f"{i['name']} — {i['price']:.0f}/{i['cost']:.0f}/"
    ...
    for m in costing.milk_economics(conn)[:5]:
        L.append(f"Молоко {m['milk']}: {round(m['share'] * 100)}% чашек, цена {m['price']} ₽, "
    ...
    if a.get("rate") is not None:
        ...
        if a.get("morning_vs_day"):
            s = a["morning_vs_day"]
            L.append(f"  утром до 11:00 — {round(s['morning'] * 100, 1)}%, "
                     f"после — {round(s['day'] * 100, 1)}%.")

    sp = staffing.shift_plan(conn)
    if sp.get("capacity", {}).get("known"):
        L.append(f"Пропускная способность: около {sp['capacity']['cups_per_hour']:.0f} "
                 f"чашек в час (собственный повторяемый максимум).")
    if sp.get("windows"):
        L.append("Часы работы на пределе: " +
                 ", ".join(f"{a2:02d}:00-{b:02d}:00" for a2, b in sp["windows"]) + ".")

    rep = supply.reorder(conn)
    if rep["items"]:
        L.append("Расход сырья в день: " +
                 "; ".join(f"{i['name']} {i['per_day']} {i['unit']}"
                           for i in rep["items"][:8]))
        urgent = [i for i in rep["items"] if i["urgency"] in ("critical", "soon")]
        if urgent:
            L.append("Заканчивается: " +
    ...
    L.append(f"Вчера ({last.isoformat()}): выручка {round(ys['revenue'])} ₽, "
    ...
    if lost["items"]:
        L.append(f"Упущенная выручка витрины (еда кончалась до закрытия): "
                 f"{lost['total_per_day']} ₽/день, около {lost['total_per_month']} ₽/мес. "
                 "По позициям: "
                 + "; ".join(f"{i['name']} — кончался {i['sellout_days']} дн., "
                             f"{i['lost_money_per_month']} ₽/мес" for i in lost["items"][:6]))
    else:
        L.append("Упущенной выручки нет: витрина до закрытия не кончалась.")
    L.append("Заказ витрины на завтра (штук): " +
             "; ".join(f"{i['name']} {i['recommended']}" for i in d["case_order_tomorrow"]))
    L.append("Важно при ответах: напитки делаются на заказ и кончиться не могут — "
