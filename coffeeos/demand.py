...
def sellout_days(conn, days=56, upto=None, rows=None):
    ...
    for r in conn.execute(
        "SELECT date, name, SUM(qty) q FROM waste "
...
def case_order(conn, target_day=None, days=56):
    ...
    stocked = conn.execute(
        "SELECT name, category FROM menu_items WHERE stocked=1 AND kind='food'"
    ).fetchall()
        ...
        order.append(
            {
                "name": name,
                "sold_avg": round(sold),
                "waste_avg": round(waste),
                "recommended": recommended,
                "note": note,
                "tag": tag,
                "demand_avg": round(st["mu"], 1),
                "sd": round(st["sd"], 1),
...
def set_order_override(conn, day, name, qty):
    ...
    conn.execute(
        "INSERT INTO case_order_override(date,name,qty) VALUES(?,?,?) "
