...
MILK_WEIGHTS = [(menu.MILK_REGULAR, 68), ("овсяное", 20), ("безлактозное", 6),
                ("миндальное", 4), ("кокосовое", 2)]
MOD_WEIGHTS = [("", 74), ("сироп", 16), ("доп. шот", 6), ("взбитые сливки", 4)]

# Профиль часа: кофейня 07:00–21:00, утренний пик и обеденная волна
HOUR_WEIGHTS = {7: 0.35, 8: 1.00, 9: 0.92, 10: 0.55, 11: 0.45, 12: 0.62, 13: 0.68,
                14: 0.48, 15: 0.40, 16: 0.44, 17: 0.52, 18: 0.46, 19: 0.30, 20: 0.18}
...
def seed(days=90, base_checks=210, seed_val=42, reset=True):
    ...
    db.init_db(conn)
    if reset:
        for t in ("receipt_items", "receipts", "menu_items", "case_order_override",
                  "stock_counts"):
    ...
    for d in range(days, 0, -1):
        ...
        for name, _price, _w in FOOD_MENU:
            want = target[name] * (DOW_FACTOR[day.weekday()] / (sum(DOW_FACTOR) / 7)) \
        ...
        events = sorted((rnd.choices(hours, weights=hweights, k=1)[0], rnd.randint(0, 59))
        ...
        for hour, minute in served:
            ...
            for name, qty, price in bought:
                ...
                conn.execute(
                    ...
                    " VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",
                    (rid, name, p["base"], p["category"], p["kind"], p["size"],
    ...
    for name, price, _w in FOOD_MENU:
        conn.execute("UPDATE ingredients SET pack_price=?, price_src='owner' WHERE name=?",
...
def _reprice_milk_waste(conn):
    row = conn.execute("SELECT pack_price, pack_qty FROM ingredients "
                       "WHERE name='Молоко обычное'").fetchone()
    if not row or not row["pack_qty"]:
        return
    per_litre = (row["pack_price"] or 0) * 1000 / row["pack_qty"]
    conn.execute("UPDATE waste SET amount = qty * ? WHERE kind='milk' AND src='demo'",
                 (per_litre,))
