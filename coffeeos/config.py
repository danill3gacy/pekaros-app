...
STAFF_CHAT_IDS = [c.strip() for c in os.getenv("STAFF_CHAT_IDS", "").split(",") if c.strip()] or list(BRIEF_CHAT_IDS)
...
BACKUP_DIR = os.getenv("BACKUP_DIR", "").split("#")[0].strip()   # пусто = папка backups рядом с базой
...
def _share_env(name, default=None):
...
def brief_hour_minute():
    h, m = BRIEF_TIME.split(":")
    return int(h), int(m)


def close_hour_minute():
...
def validate():
    ...
    if not BRIEF_CHAT_IDS:
        problems.append("Не задан BRIEF_CHAT_IDS — утренняя сводка никому не уйдёт, "
    ...
    if BOT_TOKEN and not (BRIEF_CHAT_IDS or STAFF_CHAT_IDS):
        problems.append("Токен бота задан, а список допущенных пуст: пока это так, "
                        "бот отвечает только на /start и данные закрыты.")
    if not WEB_PASSWORD:
        problems.append("Не задан WEB_PASSWORD — веб-дашборд открыт без пароля, "
    ...
    try:
        ...
        problems.append("BRIEF_TIME/CLOSE_TIME должны быть в формате ЧЧ:ММ (часы 0–23, минуты 0–59).")
    # автозагрузка
    ...
    if DB_PATH and not os.path.isabs(DB_PATH):
        problems.append(f"DB_PATH='{DB_PATH}' задан относительным путём — бот и сайт "
