...
DRINK_MENU = [
    ("Латте 350", 260, 100),
    ("Капучино 250", 200, 80),
    ("Американо 350", 170, 50),
    ("Флэт уайт 350", 280, 25),
    ("Раф 350", 320, 20),
    ("Чай 350", 170, 18),
]
...
def demand_stream(days, base_checks, seed_val):
    ...
    for idx, d in enumerate(range(days, 0, -1)):
        ...
        n_checks = int(
            base_checks * DOW_FACTOR[day.weekday()] * drift * spike * rnd.uniform(0.85, 1.15)
        )
        checks = []
...
def policy_gut(conn, day, state):
    ...
    return {
        n: max(
            1,
            int(round(target[n] * (DOW_FACTOR[day.weekday()] / avg_dow) * rnd.uniform(0.92, 1.06))),
        )
        for n in target
    }
...
def run_policy(name, plan_fn, stream, cost_share, record_waste=True):
    ...
    for idx, (day, checks) in enumerate(stream):
        ...
        for hour, minute, is_cash, drink, food in checks:
            ...
            cur = conn.execute(
                "INSERT INTO receipts(ts,payment,ext_id) VALUES(?,?,?)",
...
def run_supply_check(stream, horizon=7):
    ...
    watched = [
        n for n, r in ing.items() if r["category"] in ("coffee", "dairy", "syrup", "packaging")
    ]
    ...
    for _day, checks in stream[:warm_days]:
        for _h, _m, _c, drink, food in checks:
            ...
            for ingredient, amount in costing.portion_ingredients(
                recipes, p["base"], p["size"], p["milk"], p["mods"], p["iced"]
            ).items():
...
def main():
    ...
    ap.add_argument(
        "--supply", action="store_true", help="ещё и проверить, что заявка не даёт встать без сырья"
    )
    a = ap.parse_args()

    months = (a.days - WARMUP) / 30.0
    names = [
        "чутьё владельца (как сейчас)",
    ...
    fns = [
        (policy_gut, True),
        (policy_average, True),
        (policy_coffeeos, True),
        (policy_coffeeos, False),
    ]
    ...
    print(
        f"{'стратегия':34s} {'валовая приб.':>15} {'к «чутью»':>12} "
    ...
    for nm in names:
        ...
        print(
            f"{nm:34s} {g:14,.0f} ₽ {d:+11,.0f} ₽ {avg(acc[nm]['waste']):8,.0f} "
            f"{avg(acc[nm]['lost']):9,.0f} {avg(acc[nm]['service']) * 100:7.0f}%{spread}".replace(
                ",", " "
            )
        )
    print(
        "\nШтуки — в месяц. Списано = поставили и не продали. Недопродано = гость хотел, а не было."
    )
    print(
        "Сервис = в какой доле «позиция×день» хватило на всех. Это фактический "
    ...
    if a.supply:
        ...
        print("  Владелец каждый день смотрит заявку и берёт то, что помечено «горит» или «скоро».")
        if res["stockouts"]:
